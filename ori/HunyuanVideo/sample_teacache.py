...
def main():
    ...
    hunyuan_video_sampler.pipeline.transformer.__class__.rel_l1_thresh = 0.1 # 0.1 for 1.6x speedup, 0.15 for 2.1x speedup
    ...
    if 'LOCAL_RANK' not in os.environ or int(os.environ['LOCAL_RANK']) == 0:
        for i, sample in enumerate(samples):
            ...
            save_path = f"{save_path}/seed{outputs['seeds'][i]}_{outputs['prompts'][i][:100].replace('/','')}.mp4"
