...
frame_indices = [12, 21, 37]  # 选取的 frames
...
# 计算网格的行数和列数（调换行列）
grid_rows = len(frame_indices)  # 现在 Frame 作为行
grid_cols = len(steps)  # 现在 Step 作为列
...
for index, (img, label) in enumerate(zip(opened_images, image_labels)):
    col = index // grid_rows  # 现在 Step 作为列
    row = index % grid_rows   # 现在 Frame 作为行
